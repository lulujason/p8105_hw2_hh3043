...
```
There are `r nrow(mci)/4` participants in common. `r (nrow(amyloid)-nrow(mci))/4` participants are only in `amyloid` dataset, and `r nrow(baseline)-nrow(mci)/4` participants are only in `baseline` dataset. There are `r nrows(mci)` observations and `r ncols(mci)` variables in `baseline` dataset. For those participants who developed mci,the proportion they carry APOE4 is`r nrow(filter(mci,apoe4 == 1 & age_at_onset != '.'))/nrow(filter(mci,age_at_onset != '.'))`.
